...
datasets <- readRDS("../../2019-10-06_multivar_surv/survival_datasets.2019-10-09.rds")
...
ui <- fluidPage(
    ...
    titlePanel(div(img(src='pigeon_icon.jpg',
                       style=c("width:90px","height:90px")),
...
server <- function(input, output) {
    ...
    output$choose_probes <- renderUI({
      ...
      if (is_empty(dat) | is_empty(gene)) {
        ...
        probes <- rowData(dat)[rowData(dat)$symbol == gene,]
      }
