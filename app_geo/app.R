...
# load gene sets
gs_files <- list.files(path = "/poolio/resources/featuresets/genes",
                       pattern = "*.rdata", recursive = T, full.names = T)
lapply(gs_files, load, .GlobalEnv)
...
# function to convert a set list into a T2G 
...
ui <- fluidPage(
    ...
    titlePanel(tags$h4(tags$a(
        ...
        href="http://10.197.211.94:80"), "Gene Expression Omnibus")),
    ...
    sidebarLayout(
        ...
        sidebarPanel(width = 3,
                     ...
                     div(tags$small("Enter a GEO series accession ID (e.g. GSE55976). Press GO to update.")),
                     hr(),
                     uiOutput("geo_link"),
                     hr(),
                     uiOutput("choose_gpl"),
                     tags$small("Some data series will contain multiple different platform types,
...
server <- function(input, output) {
    ...
    # icon
    output$icon <- renderImage(list(src = "hexagons/.png",
    ...
    output$upload_text <- renderUI({
        ...
        if (is.null(gpl_data)) {
            ...
            expr_data <- exprs(gpl_data)
            if (typeof(expr_data) == "logical") {
                tags$small("If this panel is displayed it indicates that the selected
                           dataset does not provide expression data as part of the
                           GEO database. Count data may be manually uploaded if
                           available (e.g. from supplementary files on GEO page),
                           but make sure that the rownames (i.e. sample names) of
                           your count matrix match the rownames of the phenotype data
                           from the GEO database (as shown in left-most column of
                           table under the Phenotype Info tab).")
            } else {
                return()
            }
        }
